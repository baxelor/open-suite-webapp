...
@DynamicInsert
@DynamicUpdate
@Table(name = "PURCHASE_PURCHASE_ORDER_LINE", indexes = { @Index(columnList = "fullName"), @Index(columnList = "purchase_order"), @Index(columnList = "product"), @Index(columnList = "tax_line"), @Index(columnList = "tax_equiv"), @Index(columnList = "unit"), @Index(columnList = "analytic_distribution_template"), @Index(columnList = "budget"), @Index(columnList = "project"), @Index(columnList = "supplier_partner") })
public class PurchaseOrderLine extends AuditableModel {
	...
	@Widget(title = "Suppliers requests")
	@OneToMany(fetch = FetchType.LAZY, mappedBy = "purchaseOrderLine", cascade = CascadeType.ALL, orphanRemoval = true)
	private List<PurchaseOrderSupplierLine> purchaseOrderSupplierLineList;
	...
	public List<PurchaseOrderSupplierLine> getPurchaseOrderSupplierLineList() {
		return purchaseOrderSupplierLineList;
	}

	public void setPurchaseOrderSupplierLineList(List<PurchaseOrderSupplierLine> purchaseOrderSupplierLineList) {
		this.purchaseOrderSupplierLineList = purchaseOrderSupplierLineList;
	}

	/**
	 * Add the given {@link PurchaseOrderSupplierLine} item to the {@code purchaseOrderSupplierLineList}.
	 *
	 * <p>
	 * It sets {@code item.purchaseOrderLine = this} to ensure the proper relationship.
	 * </p>
	 ...
	 *            the item to add
	 */
	public void addPurchaseOrderSupplierLineListItem(PurchaseOrderSupplierLine item) {
		if (getPurchaseOrderSupplierLineList() == null) {
			setPurchaseOrderSupplierLineList(new ArrayList<>());
		}
		getPurchaseOrderSupplierLineList().add(item);
		item.setPurchaseOrderLine(this);
	}

	/**
	 * Remove the given {@link PurchaseOrderSupplierLine} item from the {@code purchaseOrderSupplierLineList}.
	 *
 	 * @param item
	 *            the item to remove
	 */
	public void removePurchaseOrderSupplierLineListItem(PurchaseOrderSupplierLine item) {
		if (getPurchaseOrderSupplierLineList() == null) {
			return;
		}
		getPurchaseOrderSupplierLineList().remove(item);
	}

	/**
	 * Clear the {@code purchaseOrderSupplierLineList} collection.
	 *
	 * <p>
	 * If you have to query {@link PurchaseOrderSupplierLine} records in same transaction, make
	 * sure to call {@link javax.persistence.EntityManager#flush() } to avoid
	 * unexpected errors.
	 * </p>
	 */
	public void clearPurchaseOrderSupplierLineList() {
		if (getPurchaseOrderSupplierLineList() != null) {
			getPurchaseOrderSupplierLineList().clear();
		}
	}
